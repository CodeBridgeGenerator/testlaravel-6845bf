...
const UsersCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            ...
            is_email_verified: _entity.is_email_verified,
            remember_token: _entity.remember_token,
            email_verified_at: _entity.email_verified_at,
        };
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div>
                <p className="m-0">is_email_verified:</p>
                <Checkbox checked={_entity?.is_email_verified} onChange={ (e) => setValByKey("is_email_verified", e.checked)}  ></Checkbox>
            </div>
            <div>
                <p className="m-0">remember_token:</p>
                <Checkbox checked={_entity?.remember_token} onChange={ (e) => setValByKey("remember_token", e.checked)}  ></Checkbox>
            </div>
